...
function FeaturePlatform(p: { name: string; supported: "yes" | "no" | "warning"; lang: string }) {
  ...
  if (p.supported === "yes") {
    return (
      <div className={styles.platform_support} aria-label={t.features.featurePlatforms.availableOn(p.name)}>
        <PlatformSupportedIcon /> {p.name}
      </div>
    );
  } else if (p.supported === "warning") {
    return (
      <div className={styles.platform_support} aria-label={t.features.featurePlatforms.availableWithWarningFor(p.name)}>
        <PlatformWarningIcon /> {p.name}
      </div>
    );
  } else {
    return (
      <div className={styles.platform_support} aria-label={t.features.featurePlatforms.unavailableOn(p.name)}>
...
export default function FeaturePlatforms({
  platforms,
  lang,
  inline,
}: {
